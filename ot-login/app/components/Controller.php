...
class Controller
{
	...
	public function __construct($container)
	{
        ...
        //$this->auth_token = $this->request->getHeader('X-OT-ACCESS-TOKEN');
        //$this->request_source = $this->request->getHeader('X-Request-Source');
	}
            ...
            if (isset($rule['type'])) 
            {
                if (isset($request_params[$param]) && !empty($request_params[$param])) 
                {
                    ...
                    switch ($rule['type']) 
                    {
                        ...
                        case 'ip':
                            if(!filter_var($request_params[$param], FILTER_VALIDATE_IP))
                            {
                                return $this->throwError($param . ' value must be a valid ip');
                            }
                            break;
                    }
                }
